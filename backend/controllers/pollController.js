...
exports.getPollById = async (req, res) => {
  ...
  try {
    ...
    if (!poll) {
      return res.status(404).json({ message: "Error registering user", error: err.message });
    }
...
exports.bookmarkPoll = async (req, res) => {
  ...
  try {
    ...
    await user.save();
    res.status(200).json({ message: "Poll bookmarked successfully", bookmarkedPolls: user.bookmarkedPolls });
  } catch (err) {
    res
      .status(500)
      .json({ message: "Error registering user", error: err.message });
